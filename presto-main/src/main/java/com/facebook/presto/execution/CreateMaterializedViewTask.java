        ...
        implements DataDefinitionTask<CreateMaterializedView>
{
    ...
    public ListenableFuture<?> execute(CreateMaterializedView statement, TransactionManager transactionManager, Metadata metadata, AccessControl accessControl, QueryStateMachine stateMachine, List<Expression> parameters)
    {
        ...
        List<ConnectorMaterializedViewDefinition.ViewColumn> columns = columnMetadata.stream()
                .map(column -> new ConnectorMaterializedViewDefinition.ViewColumn(column.getName(), column.getType()))
                .collect(toImmutableList());

        List<ConnectorMaterializedViewDefinition.ViewBaseTable> baseTables = analysis.getTableNodes().stream()
                .map(table -> {
                    ...
                    return new ConnectorMaterializedViewDefinition.ViewBaseTable(toSchemaTableName(tableName));
                })
                .collect(toImmutableList());

        ConnectorMaterializedViewDefinition viewDefinition = new ConnectorMaterializedViewDefinition(
                sql,
                Optional.of(viewName.getCatalogName()),
