        ...
        implements DataDefinitionTask<CreateMaterializedView>
{
    ...
    public ListenableFuture<?> execute(CreateMaterializedView statement, TransactionManager transactionManager, Metadata metadata, AccessControl accessControl, QueryStateMachine stateMachine, List<Expression> parameters)
    {
        ...
        List<MaterializedViewDefinition.ViewColumn> columns = columnMetadata.stream()
                .map(column -> new MaterializedViewDefinition.ViewColumn(column.getName(), column.getType()))
                .collect(toImmutableList());

        List<MaterializedViewDefinition.ViewBaseTable> baseTables = analysis.getTableNodes().stream()
                .map(table -> {
                    ...
                    return new MaterializedViewDefinition.ViewBaseTable(toSchemaTableName(tableName));
                })
                .collect(toImmutableList());
