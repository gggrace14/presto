...
import static com.facebook.presto.sql.analyzer.SemanticErrorCode.UNSUPPORTED_MATERIALIZED_VIEW;
        ...
        implements DataDefinitionTask<CreateMaterializedView>
{
    ...
    public ListenableFuture<?> execute(CreateMaterializedView statement, TransactionManager transactionManager, Metadata metadata, AccessControl accessControl, QueryStateMachine stateMachine, List<Expression> parameters)
    {
        ...
        validateMaterializedView(statement, analysis);
        ...
        ConnectorMaterializedViewDefinition viewDefinitionWithColumnMap = viewDefinition.withViewToBaseColumnMap(analysis.getOriginalColumnMapping(statement.getQuery()));
        try {
            metadata.createMaterializedView(session, viewName.getCatalogName(), viewMetadata, viewDefinitionWithColumnMap, statement.isNotExists());
        }
    ...
    private static void validateMaterializedView(CreateMaterializedView statement, Analysis analysis)
    {
        ...
        if (analysis.hasNamedQueries()) {
            throw new SemanticException(UNSUPPORTED_MATERIALIZED_VIEW, query, "With in materialized view is not supported yet.");
        }
        if (analysis.hasWindowFunctions()) {
            throw new SemanticException(UNSUPPORTED_MATERIALIZED_VIEW, query, "WindowFunction in materialized view is not supported yet.");
        }
        if (analysis.hasSubqueries()) {
            throw new SemanticException(UNSUPPORTED_MATERIALIZED_VIEW, query, "Subqueries in materialized view is not supported yet.");
        }
    }
}
