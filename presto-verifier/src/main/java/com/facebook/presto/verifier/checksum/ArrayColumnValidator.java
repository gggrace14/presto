        ...
        implements ColumnValidator
{
    ...
    public List<ColumnMatchResult<ArrayColumnChecksum>> validate(Column column, ChecksumResult controlResult, ChecksumResult testResult)
    {
        ...
        boolean useStringAsDoublePath = useStringAsDoublePath(column);
        ...
        if (useStringAsDoublePath(column) && ColumnValidatorUtil.isDoubleAsStringColumn(column, controlResult, testResult)) {
    ...
    public static List<SingleColumn> generateStringArrayChecksumColumns(Column column)
    {
        ...
        Expression nullCount = functionCall(
                "sum",
                functionCall(
                        "cardinality",
                        functionCall(
                                ...
                                new LambdaExpression(
                                        ...
                                        new IsNullPredicate(identifier("x"))))));
        Expression asDoubleNullCount = functionCall(
                "sum",
                functionCall(
                        "cardinality",
                        functionCall(
                                "filter",
                                getAsDoubleArrayExpression(column),
                                new LambdaExpression(
                                        ImmutableList.of(new LambdaArgumentDeclaration(identifier("x"))),
                                        new IsNullPredicate(identifier("x"))))));

        Column asDoubleArrayColumn = getAsDoubleArrayColumn(column);
        builder.addAll(generateFloatingPointArrayChecksumColumns(asDoubleArrayColumn));
        builder.add(new SingleColumn(nullCount, delimitedIdentifier(ColumnValidatorUtil.getNullCountColumnAlias(column))));
        builder.add(new SingleColumn(asDoubleNullCount, delimitedIdentifier(ColumnValidatorUtil.getAsDoubleNullCountColumnAlias(column))));
    ...
    private ArrayColumnChecksum toColumnChecksum(Column column, ChecksumResult checksumResult, boolean useFloatingPointPath, boolean useStringAsDoublePath)
    {
    ...
    {
        return Column.create(column.getName() + "_as_double", getAsDoubleArrayExpression(column), DOUBLE);
    }
