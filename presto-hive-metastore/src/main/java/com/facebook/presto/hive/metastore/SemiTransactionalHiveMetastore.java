...
public class SemiTransactionalHiveMetastore
{
    ...
    public synchronized void updateTableParameters(ConnectorSession session, String databaseName, String tableName, Map<String, String> parametersToUpdate)
    {
        setShared();
        ...
        if (oldTableAction == null) {
            ...
            for (Map.Entry<String, String> entry : oldTable.getParameters().entrySet()) {
                if (!parametersToUpdate.containsKey(entry.getKey())) {
                    parameters.put(entry.getKey(), entry.getValue());
                }
            }
            Table newTable = Table.builder(oldTable).setParameters(parameters.build()).build();
    ...
    private static class UpdateTableParametersOperation
    {
        ...
        public void run(ExtendedHiveMetastore metastore)
        {
            Set<String> toDrop = difference(oldParameters.keySet(), newParameters.keySet());
            metastore.updateTableParameters(tableName.getSchemaName(), tableName.getTableName(), newParameters, toDrop);
            done = true;
        }
        ...
        public void undo(ExtendedHiveMetastore metastore)
        {
            ...
            metastore.updateTableParameters(tableName.getSchemaName(), tableName.getTableName(), oldParameters, toDrop);
            done = false;
        }
    }
